...
def render_percept_overlay(
    ...
    adjacent = [
        (ax, ay) for ax, ay in adjacent if 1 <= ax <= width and 1 <= ay <= height
    ]
    ...
    for ax, ay in adjacent:
        ...
        if 0 <= grid_row < len(new_grid) and 0 <= grid_col < len(new_grid[0]):
            ...
            if percept.creaking and percept.rumbling:
                overlay = tuple(
                    (c1 + c2) / 2
                    for c1, c2 in zip(COLORS["creaking"], COLORS["rumbling"])
                )
            ...
            # Alpha blend
            blended = tuple(
                base_color[i] * (1 - alpha) + overlay[i] * alpha for i in range(3)
            )
...
def plot_state(
    ...
    # Get grid colors
    grid = state_to_grid(
        env, reveal=reveal, known_safe=known_safe, known_dangerous=known_dangerous
    )
    if show_percepts:
...
def create_grid_figure(plt, env, output_path):
    ...
    for y in range(1, height + 1):
        for x in range(1, width + 1):
            ...
            if pos in damaged:
                ...
                # Wheel
                wheel = plt.Circle(
                    (cx - 0.05, cy - 0.18), 0.08, facecolor="black", edgecolor="black"
                )
                ax.add_patch(wheel)
            elif pos == package:
    ...
    legend_elements = [
        ...
        Patch(
            facecolor=COLORS["forklift"], edgecolor="gray", label="Forklift (forklift)"
        ),
...
def create_reasoning_step_figure(
    ...
    fig, (ax_agent, ax_true) = plt.subplots(
        1, 2, figsize=(12, 5.5), gridspec_kw={"wspace": 0.15}
    )
            ...
            for x in range(1, width + 1):
                ...
                if view_type == "agent":
                    ...
                    if pos in known_rumbling:
                        ...
                        for i in range(5):
                            ...
                            ax.plot(
                                [
                                    plot_x + 1 - max(0, offset),
                                    plot_x + 1 - min(1, offset + 1),
                                ],
                                [plot_y + max(0, -offset), plot_y + min(1, 1 - offset)],
                                color=COLORS["rumbling"],
                                linewidth=6,
                ...
                if view_type == "true" and pos != (robot["x"], robot["y"]):
                    ...
                    if pos in damaged:
                        ...
                        # Ribbon/tape
                        ax.plot(
                            [cx - 0.2, cx + 0.2], [cy, cy], color="#8B4513", linewidth=2
                        )
                        ax.plot(
                            [cx, cx],
                            [cy - 0.15, cy + 0.15],
...
def save_frames(
    ...
    for i, state in enumerate(history):
        ...
        fig.savefig(
            Path(output_dir) / f"frame_{i:04d}.png", dpi=dpi, bbox_inches="tight"
        )
        plt.close(fig)
